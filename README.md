...
страницы устройства. В манифесте основная запись образа — сжатая: `file`,
`size` и `md5` описывают `.bin.gz`, несжатый проходит полями `raw`, `raw_file`,
`raw_md5`. Шаги публикации стоят в workflow после создания релиза,
